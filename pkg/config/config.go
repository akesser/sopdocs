...
type DocConfig struct {
	...
	TokenVariables       *map[string]string `yaml:"tokenVariables,omitempty"`
}
...
func LoadConfigWithDefaults(filepath string) (*DocConfig, error) {
	...
	defaultConfig := DocConfig{
		...
		TokenVariables:       &map[string]string{},
	}
...
func LoadTokensFromEnv(tokenConfig *map[string]string) map[string]string {
	tokens := map[string]string{}
	for key, env := range *tokenConfig {
		tokens[strings.ToLower(key)] = os.Getenv(env)
	}
...
func (in *DocConfig) DeepCopyInto(out *DocConfig) {
	*out = *in
	...
	if in.TokenVariables != nil {
		in, out := &in.TokenVariables, &out.TokenVariables
